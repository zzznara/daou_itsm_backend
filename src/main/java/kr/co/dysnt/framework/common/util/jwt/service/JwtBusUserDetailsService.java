...
@Slf4j
@Service("jwtBusUserDetailsService")
@RequiredArgsConstructor
public class JwtBusUserDetailsService implements UserDetailsService {
    ...
    public TmsUser loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        try {
            ...
            // authVo = authService.getBusUser(paramMap);
            TmsUser tmsUser = (TmsUser) tmsUserDetailsService.loadUserByUsername(username);
            return tmsUser;

        } catch (Exception e) {
            log.error("[BUS LOGIN] JwtBusUserDetailsService loadUserByUsername Exception : {}", e.getMessage());
